...
**当前阶段：Milestone 1 · 阶段 1 + 阶段 2 + 阶段 3（服务端）已完成** 🚧

已落地：`backend/` 工程骨架（FastAPI + SQLAlchemy 2.0 async）、`app/core/balance.py` 数值唯一出处、
**16 张表** ORM 模型与建表脚本、确定性离线结算引擎、新游戏初始化流程，以及 5 个接口：
`GET /colony/state`、`POST /colony/snapshot`（含对账协议）、`POST /colony/scavenge`（冷启动手点废墟）、
`POST /colony/dispatch`（工位调度）、`POST /facilities/build`（设施建造）。
冷启动闭环已端到端跑通：**手点废墟 → 第一座纸箱窝引来第一只猫 → 农夫上工 → 第 2 只猫 → 拾荒猫接管**。
前端（阶段 2）已落地：Vue 3 + Tailwind + Pinia 的极客终端界面、100ms 平滑插值 + 15s 静默快照双时钟、
HUD 双条（资源 / 警戒与电力）、中央终端日志流、设施建造与工位调度面板、开局冷启动五步引导。
后端已接上**星区推演导演**（LLM）：公频电台上台语料走本地狂欢节拼装（0 Token），
底栏「生成语料」按钮触发一次小批量 LLM 写稿（默认便宜小模型 + 每日预算熔断 + 三道防火墙）。
**科技树（模块 E）已上线**：19 节点 DAG、算力自动研发、科研等级折扣（Tier 2 八折 / Tier 3 六折）、
进阶设施的科技建造门槛与「重 Roll」；左栏为「设施建造 + 科技树」双面板。
...
| [代码结构与核心工程详细设计规范.md](代码结构与核心工程详细设计规范.md) | 前后端目录树、Pinia 分仓、API 定稿（34 个接口）、环境变量与 LLM 策略 | 写代码之前 |
