...
**当前阶段：Milestone 1 模块 A ~ O 全部落地，可完整通关** ✅　—　进度与后续计划见 [开发进展与后续计划.md](开发进展与后续计划.md)

后端：`backend/` 工程骨架（FastAPI + SQLAlchemy 2.0 async）、`app/core/balance.py` 数值唯一出处、
**16 张表** ORM 模型与建表脚本、确定性离线结算引擎、新游戏初始化流程，共 **45 个接口**（文档清单 45 = 代码 45，
由 `scripts/api_doc_consistency.py` 逐条比对）——离线结算与对账、读档（`/game/load`）、
冷启动手点废墟、工位调度、设施建造、19 节点科技树、军备与远征、猫草水培基因实验室、智械匿名深网、
星球小游戏（密电译码 / 矿脉扫描 / 熔炉配比）、发射井与星图、**外星球基地建行与跨星迁猫航线**、欧米茄终局决战、生涯成就、多槽位存档导出导入。
冷启动闭环已端到端跑通（真实浏览器实测）：**手点废墟 → 第一座纸箱窝引来第一只猫 → 农夫上工 → 第 2 只猫 → 拾荒猫接管**。

前端：Vue 3 + Tailwind + Pinia 的极客终端界面、100ms 平滑插值 + 15s 静默快照双时钟、
HUD 双条（资源 / 警戒与电力）、中央终端日志流、公频电台底栏，左栏八个面板（设施建造 / 科技树 /
猫草实验室 / 智械深网 / 小游戏 / 生涯成就 / 星区星图 / 存档槽位）+ 右栏（工位调度 / 战备机库）。

**星区推演导演**（LLM，6 个场景全部接入）：新行星生态标签、外星球特化科技卡（一次批量铺满 12 节点）、
公频电台语料批处理、论坛发帖裁判、《异星植物学图鉴》论文、通关碑文；
...
| [代码结构与核心工程详细设计规范.md](代码结构与核心工程详细设计规范.md) | 前后端目录树、Pinia 分仓、API 定稿（45 个接口，见 §4）、环境变量与 LLM 策略 | 写代码之前 |
| [开发进展与后续计划.md](开发进展与后续计划.md) | 现状数字（从代码算）、最近迭代、四道收尾关卡、后续计划 P0~P2 与待拍板项 | 想知道"现在到哪了、接下来做什么" |
...
| v1.6 | 2026-09-13 | LLM 场景 5（《异星植物学图鉴》论文）落地：六个 LLM 场景全部接入，README 不再有"未实现场景"；猫草实验室新增论文折叠区 |
| v1.7 | 2026-09-13 | `GET /game/load` 读档接口落地：接口清单 **68/68 全部实现**；读档不自动建档、读档即激活目标星球，外星球基地未实现时如实拒绝 |
| v1.8 | 2026-09-13 | 星际第①/②步落地：外星球基地建行（解锁后切星/读档即建行，锚点写"解锁那一刻"）+ 跨星迁猫航线（`POST /planet/migrate`，被劫掠只延误不丢猫）；接口总数 68 → **69**，README 首页接口清单措辞同步 |
| v1.9 | 2026-09-13 | 星际第③步（星球性格：承载力 / 产粮 / 专属产出）落地 + 新增「星区星图」面板（左栏八个 Tab），星际内容前后端闭环 |
