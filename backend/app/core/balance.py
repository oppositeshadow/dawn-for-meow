...
TECH_TIER_COSTS: dict[int, tuple[float, ...]] = {
    # v1.44 重排：前段拉长、T4 削峰（见 §6.1 的曲线形状说明）
    1: (300.0, 600.0, 1100.0, 1600.0),
    2: (1200.0, 1900.0, 2600.0, 3300.0),
    3: (3000.0, 4000.0, 4500.0, 5000.0, 6000.0),
    4: (6000.0, 8000.0, 9500.0, 11000.0, 12500.0, 13000.0),
}
PLANET0_TECH_NODE_COUNT = 19
#: 母星全科技合计（280 + 1,500 + 11,500 + 200,000）
#: 母星 19 节点合计算力（《数值平衡表》§6.1 v1.44 重排：把 T4 的时间分摊到前三段，
#: 避免"前三段几小时跑完、最后一段 30+ 小时"的削峰形状）
PLANET0_TOTAL_RESEARCH = 95_100.0
